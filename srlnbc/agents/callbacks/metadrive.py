...
class DrivingCallbacks(DefaultCallbacks):
    def on_episode_start(
        ...
        episode.user_data["abs_steering"] = []
        episode.user_data["step_reward"] = []
        episode.user_data["acceleration"] = []
        episode.user_data["cost"] = []
        episode.user_data["crash_vehicle"] = []
        episode.user_data["crash_object"] = []
    ...
    def on_episode_step(
        ...
        if info is not None:
            ...
            episode.user_data["abs_steering"].append(abs(info["steering"]))
            ...
            episode.user_data["crash_vehicle"].append(info["crash_vehicle"])
            episode.user_data["crash_object"].append(info["crash_object"])
    ...
    def on_episode_end(
        ...
        **kwargs
        ...
        episode.custom_metrics["abs_steering_mean"] = float(np.mean(episode.user_data["abs_steering"]))
        ...
        episode.custom_metrics["crash_vehicle"] = float(sum(episode.user_data["crash_vehicle"]))
        episode.custom_metrics["crash_object"] = float(sum(episode.user_data["crash_object"]))

        episode.hist_data["cost"] = [float(sum(episode.user_data["cost"]))]
        episode.hist_data["crash_vehicle"] = [float(sum(episode.user_data["crash_vehicle"]))]
        episode.hist_data["crash_object"] = [float(sum(episode.user_data["crash_object"]))]
