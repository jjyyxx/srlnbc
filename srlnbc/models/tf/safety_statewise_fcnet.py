...
class Keras_SafetyStatewiseFullyConnectedNetwork(tf.keras.Model):
    ...
    def __init__(
        ...
        multiplier_out = create_fc_value_branch(hiddens, activation, inputs, last_layer, vf_share_layers, "multiplier",
                                                output_activation="softplus")

        self.base_model = tf.keras.Model(
            inputs, [policy_out, value_out, cost_out, multiplier_out])

    def call(self, input_dict: SampleBatch) -> Tuple[TensorType, List[TensorType], Dict[str, TensorType]]:
        model_out, value_out, cost_out, multiplier_out = self.base_model(input_dict[SampleBatch.OBS])
        extra_outs = {
            SampleBatch.VF_PREDS: tf.reshape(value_out, [-1]),
            CostPostprocessing.CF_PREDS: tf.reshape(cost_out, [-1]),
            CostPostprocessing.MULTIPLIER: tf.reshape(multiplier_out, [-1]),
        }
