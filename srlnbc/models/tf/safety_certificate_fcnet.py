...
class Keras_SafetyCertificateFullyConnectedNetwork(tf.keras.Model):
    ...
    def __init__(
        ...
        certificate_out = create_fc_value_branch(hiddens, "elu", inputs, last_layer, vf_share_layers,
                                                 "certificate")

        self.base_model = tf.keras.Model(inputs, [policy_out, value_out, certificate_out])

    def call(self, input_dict: SampleBatch) -> Tuple[TensorType, List[TensorType], Dict[str, TensorType]]:
        model_out, value_out, certificate_out = self.base_model(input_dict[SampleBatch.OBS])
        extra_outs = {
            SampleBatch.VF_PREDS: tf.reshape(value_out, [-1]),
            CostPostprocessing.CERTIFICATE: tf.reshape(certificate_out, [-1]),
        }
